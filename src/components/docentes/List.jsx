...
const List = ({ docentes, setDocentes, setModo, editarDocenteRef, eliminarDocenteRef, setOpenDialogDelete }) => {
    ...
    { field: 'celular', headerName: 'Celular', minWidth: 100 },
    {
      ...
      getActions: (params) => [
        <GridActionsCellItem
          ...
          label='Eliminar'
          onClick={() => {
            eliminarDocenteRef.current = params.row;
        ...
        <GridActionsCellItem
          ...
          label='Editar'
          onClick={() => {
            editarDocenteRef.current = params.row;
            setModo('editar');
          }}
          showInMenu
        />,
  ...
  useEffect(() => {
    const getDocentes = async () => {
      try {
        const response = await fetchApi(getToken()).get('/docentes');
        let listDocentes = response.data?.docentes;
        setDocentes(listDocentes ?? []);
        if (!listDocentes || listDocentes.length === 0) {
          dataAlert.current = {msg: 'No hay docentes para mostrar', severity: 'info'};
          setOpenAlert(true);
        }
      } catch (err) {
        console.log(err);
