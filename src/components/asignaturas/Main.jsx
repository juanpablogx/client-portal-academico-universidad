...
const Main = () => {
  ...
  const editarAsignatura = useRef(null);
  const eliminarAsignatura = useRef(null);
  ...
  const main = (
    <>
      <Typography variant='h4' component='h2' sx={{ mb: 2 }}>Asignaturas</Typography>
      {modo === 'crear' ? <Form onReturn={() => setModo('listar')} /> : null}
      {modo === 'editar' ? <Form onReturn={() => setModo('listar')} asignatura={editarAsignatura.current} /> : null}
      {modo === 'listar' ? 
        <>
          <List 
            setModo={setModo}
            ...
            editarAsignaturaRef={editarAsignatura}
            eliminarAsignaturaRef={eliminarAsignatura}
      ...
      <DialogDelete 
        open={openDialog} 
        asignatura={eliminarAsignatura.current} 
        onClose={() => {
          setModo('listar')
          setOpenDialog(false);
        }} 
